...
def generate_signal(params: BaseSigParameters) -> tuple[np.ndarray, np.ndarray]:
    t = np.arange(0, params.duration, params.sample_period)
    
    if isinstance(params, SineSigParameters):
        logger.info(f"Generating Sine Wave with frequency: {params.frequency} Hz and phase: {params.phase} for {params.duration} seconds")
        return params.amplitude * np.sin(2 * np.pi * params.frequency * t), t
    
    elif isinstance(params, RampSigParameters):
        logger.info(f"Generating Ramp Wave with slope: {params.slope} for {params.duration} seconds")
        return params.slope * t, t
    
    elif isinstance(params, ChirpSigParameters):
        logger.info(f"Generating Chirp Wave from {params.f0} Hz to {params.f1} Hz for {params.duration} seconds")
